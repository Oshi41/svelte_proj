...
export const repeat = (elem, count)=>{
    const arr = [elem];
...
export const debounce = (fn, {timeout = 300} = {}) => {
    ...
    const force_fn = (...args) => {
        if (is_running())
            return;
        cancel();
        promise = promisify(fn, ...args).catch(e => {
            throw e;
        }).finally(finish);
    };
    const result = (...args) => {
        if (is_running())
            return;
        cancel();
        t = setTimeout(() => force_fn(...args), timeout);
    };
...
export const cached = (fn, {timeout = 5 * 1000, debounce: db = 300}) => {
    ...
    fn = (...args) => promisify(fn, ...args).then(x => value = x).catch(e => {
        throw e
    });
...
export const maxBy = (arr, fn) => {
    fn = fn || (x => x);
    return arr.sort((l, r) => fn(l) - fn(r))?.pop();
};
